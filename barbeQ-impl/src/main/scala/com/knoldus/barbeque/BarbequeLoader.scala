package com.knoldus.barbeque

import com.knoldus.barbeque.dao.api.BarbequeReadDAO
import com.knoldus.barbeque.dao.impl.BarbequeReadDAOImpl
import com.knoldus.barbeque.entity.BarbequeEntity
import com.knoldus.barbeque.es.BarbequeEventSourcingProcessor
import com.knoldus.barbeque.readside.BarbequeReadSide
import com.lightbend.lagom.scaladsl.api.Descriptor
import com.lightbend.lagom.scaladsl.broker.kafka.LagomKafkaComponents
import com.lightbend.lagom.scaladsl.client.ConfigurationServiceLocatorComponents
import com.lightbend.lagom.scaladsl.devmode.LagomDevModeComponents
import com.lightbend.lagom.scaladsl.persistence.cassandra.CassandraPersistenceComponents
import com.lightbend.lagom.scaladsl.server.{LagomApplication, LagomApplicationContext, LagomApplicationLoader, LagomServer}
import com.softwaremill.macwire.wire
import play.api.libs.ws.ahc.AhcWSComponents

class BarbequeLoader extends LagomApplicationLoader {
  override def load(context: LagomApplicationContext): LagomApplication =
    new BarbequeApplication(context) with ConfigurationServiceLocatorComponents {}

  override def loadDevMode(context: LagomApplicationContext): LagomApplication =
    new BarbequeApplication(context) with LagomDevModeComponents

  override def describeService: Option[Descriptor] = Some(readDescriptor[BarbequeService])
}

abstract class BarbequeApplication(context: LagomApplicationContext)
  extends LagomApplication(context)
    with CassandraPersistenceComponents
    with AhcWSComponents
    with LagomKafkaComponents {

  // Bind the service that this server provides
  override lazy val lagomServer: LagomServer = serverFor[BarbequeService](wire[BarbequeServiceImpl])
  override lazy val jsonSerializerRegistry: BarbequeSerializerRegistry.type = BarbequeSerializerRegistry

  // Initialise everything
  persistentEntityRegistry.register(wire[BarbequeEntity])

  // Registering the Read-side
  readSide.register(wire[BarbequeReadSide])

  lazy val esService: BarbequeEventSourcingProcessor = wire[BarbequeEventSourcingProcessor]
  lazy val barbequeReadDAOImpl: BarbequeReadDAO = wire[BarbequeReadDAOImpl]
}
